...
import (
	...
	client "github.com/vitwit/witvalchain/x/group/client/cli"
)
	...
	out, err = cli.ExecTestCLICmd(val.ClientCtx, client.MsgCreateGroupPolicyCmd(),
		append(
			[]string{
				...
				"1",
				validMetadata,
				testutil.WriteToNewTempFile(s.T(), fmt.Sprintf(`{"@type":"/witvalchain.group.v1.PercentageDecisionPolicy", "percentage":"%f", "windows":{"voting_period":"30000s"}}`, percentage)).Name(),
			},
			s.commonFlags...,
		),
	)
...
func (s *IntegrationTestSuite) TestTxCreateGroupWithPolicy() {
	...
	thresholdDecisionPolicyFile := testutil.WriteToNewTempFile(s.T(), `{"@type": "/witvalchain.group.v1.ThresholdDecisionPolicy","threshold": "1","windows": {"voting_period":"1s"}}`)
		...
		},
		{
			...
			append(
				[]string{
					...
					fmt.Sprintf("%v", groupID),
					validMetadata,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.PercentageDecisionPolicy", "percentage":"0.5", "windows":{"voting_period":"1s"}}`).Name(),
				},
				s.commonFlags...,
			),
			false,
		...
		},
		{
			...
			append(
				[]string{
					...
					fmt.Sprintf("%v", groupID),
					validMetadata,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.PercentageDecisionPolicy", "percentage":"2", "windows":{"voting_period":"1s"}}`).Name(),
				},
				s.commonFlags...,
			),
			true,
	...
	}{
		{
			...
			append(
				[]string{
					groupPolicy.Admin,
					groupPolicy.Address,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.ThresholdDecisionPolicy", "threshold":"1", "windows":{"voting_period":"40000s"}}`).Name(),
				},
				commonFlags...,
			),
			false,
		...
		},
		{
			...
			append(
				[]string{
					groupPolicy.Admin,
					groupPolicy.Address,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.PercentageDecisionPolicy", "percentage":"0.5", "windows":{"voting_period":"40000s"}}`).Name(),
				},
				commonFlags...,
			),
			false,
		...
		},
		{
			...
			append(
				[]string{
					groupPolicy.Admin,
					groupPolicy.Address,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.ThresholdDecisionPolicy", "threshold":"1", "windows":{"voting_period":"50000s"}}`).Name(),
					fmt.Sprintf("--%s=%s", flags.FlagSignMode, flags.SignModeLegacyAminoJSON),
				},
				commonFlags...,
			),
		...
		},
		{
			...
			append(
				[]string{
					newAdmin.String(),
					groupPolicy.Address,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.ThresholdDecisionPolicy", "threshold":"1", "windows":{"voting_period":"1s"}}`).Name(),
				},
				commonFlags...,
			),
			true,
		...
		},
		{
			...
			append(
				[]string{
					groupPolicy.Admin,
					groupPolicy.Address,
					testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.PercentageDecisionPolicy", "percentage":"2", "windows":{"voting_period":"40000s"}}`).Name(),
				},
				commonFlags...,
			),
			true,
	...
	out, err = cli.ExecTestCLICmd(clientCtx, client.MsgCreateGroupPolicyCmd(),
		append(
			[]string{
				...
				groupID,
				"AQ==",
				testutil.WriteToNewTempFile(s.T(), `{"@type":"/witvalchain.group.v1.ThresholdDecisionPolicy", "threshold":"3", "windows":{"voting_period":"1s"}}`).Name(),
			},
			s.commonFlags...,
		),
	)
	...
	out, err := cli.ExecTestCLICmd(clientCtx, client.MsgCreateGroupPolicyCmd(),
		append(
			[]string{
				...
				groupID,
				validMetadata,
				testutil.WriteToNewTempFile(s.T(), fmt.Sprintf(`{"@type":"/witvalchain.group.v1.ThresholdDecisionPolicy", "threshold":"%d", "windows":{"voting_period":"30000s"}}`, threshold)).Name(),
			},
			s.commonFlags...,
		),
	)
